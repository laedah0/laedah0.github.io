...
{		
	...
	//수량 인풋텍스트 타입
	orderListHtml += '<input style="width:200px" type="text" id="orderTxt'+id+'_1" placeholder="옵션명" onfocus="this.select()"/>&nbsp;';
	orderListHtml += '<input style="width:75px" placeholder="단가" type="text" id="orderTxt'+id+'_2" onfocus="this.select()"/>&nbsp;';
	orderListHtml += '<input style="width:75px" placeholder="할인율(%)" type="text" id="orderTxt'+id+'_4" onfocus="this.select()"/>&nbsp;X&nbsp;';
	orderListHtml += '<input type="text" style="width:40px" placeholder="수량" id="orderTxt'+id+'_3" value="1" onfocus="this.select()"/>개</br>';
...
*/
	...
	for(var i=0; i<itemLength; i++)
	{
		...
		} else
		{
			if(isEmpty(orderTxt4.value))
			{
				...
				//할인율 적용
				var salePrice = parseInt(orderTxt2.value) * 0.01 * parseInt(orderTxt4.value);
				orderListTxt += orderTxt1.value + '(단가 ' + comma(orderTxt2.value)+'->'+salePrice+orderTxt4.value+'%할인가' + '원) ' + orderTxt3.value + '개, ';
				totalPrice += salePrice * parseInt(orderTxt3.value);
				totalPriceTxt += '(' + orderTxt2.value + 'x -' + orderTxt4.value + '% x' + parseInt(orderTxt3.value) + ')+';
			}
		}
	}
	...
	returnTxt = '[미건철물장식] ' + orderListTxt + '배송비(' + deleveryPriceTxt + ') 및 설치용 피스 포함, 총 '+ comma(totalPrice);
	returnTxt += '원입니다.\n입금계좌는 기업 010-2794-9795 박상은 입니다.\n확인해보시고 입금하신 후 받으실 분 성함/주소/연락처 부탁드립니다^^ 감사합니다!';
...
var orderSendInfoTemplate = function(_num,_cop)
{
	...
	switch(sendType.checkedValue())
	{
		...
		case "long-noreturn":
			//오배송 수거없음 송장안내
			returnTxt = '[미건철물장식] 제품 문제로 불편을 드려 죄송합니다. 해당 상품은 새 상품으로 다음과 같이 재발송 됩니다.\n아울러 기존 파손 상품은 배송비 문제로 별도 수거 없이 파기 부탁드리오니 너른 양해 부탁드립니다.\n\n';
			returnTxt += copName + ' ' + replaceNum; 
...
var sweettracker = function()
{
	...
	{
		return href = this.url + '?t_key=' + this.key + '&t_code=' + this.code + '&t_invoice='+ this.invoice; 
	}
...
{		
	...
	if(_key)
	{
		...
		**/
		...
		//수량 인풋텍스트박스
		missingReturnListHtml += '<input style="width:200px" type="text" id="missingReturnTxt'+id+'_1" placeholder="옵션명" onfocus="this.select()"/>&nbsp;';
		missingReturnListHtml += '<input style="width:50px" placeholder="단가" type="text" id="missingReturnTxt'+id+'_2" onfocus="this.select()"/>&nbsp;X&nbsp;';
		missingReturnListHtml += '<input type="text" style="width:40px" id="missingReturnTxt'+id+'_3" placeholder="수량" onfocus="this.select()" value="1"/>개';
...
{		
	...
	if(deleveryPrice == 0)
	{
		...
		returnTxt += '반품배송비는 요청에 따라 환불금에서 자동 차감 예정입니다. 실제 환불일은 네이버쇼핑 결제방식에 따라 달라질 수 있으며 관련 궁금하신 점은 네이버쇼핑 고객센터로 문의주세요.\n\n';
		...
		returnTxt += '\n입금계좌 : 기업 010-2794-9795 박상은, 금액 총'+comma(totalPrice)+'원\n\n';
		returnTxt += '환불금은 주문시 지불하신 배송비 포함 결제금액 전액 환불 예정입니다. 실제 환불일은 네이버쇼핑 결제방식에 따라 달라질 수 있으며 관련 궁금하신 점은 네이버쇼핑 고객센터로 문의주세요.\n\n';
		returnTxt += '확인해 보시고 관련 의문사항이 있으시면 문의 남겨주세요. 감사합니다!';
	}
...
var callHelpCenter = function(_num)
{
	...
	if (confirm('CJ대한통운 고객센터 연결('+number+')') == true){    //확인
		//롯데택배
...
var goDirectAsKey = function(_key)
{
	...
	switch(_key)
	{
		...
		case 'chatbot' :
			//쳇봇상담 바로가기
			_url ='https://chatbot.cjlogistics.com:8443/chatbot/?sessionKey=qbmkel9gcd0t1d6bi7hzz4q9p&company=0000000000&media=COM&agent=Chrome';
			break;
...
var searchOrderDetail = function(_type, _keyword)
{
	...
	location.href = 'https://sell.smartstore.naver.com/o/v3/iframe/manage/order?detailSearch.type='+type+'&detailSearch.keyword='+keyword;
...
var viewOrderDetailViewPopup = function(_num)
{
	...
	if(isKO(_num) || isEN(_num) || isSpecific(_num) || _num.length != 16) {
		...
		location.href='https://sell.smartstore.naver.com/o/orderDetail/productOrder/'+_num+'/popup'; 
	}
	return null;
};
