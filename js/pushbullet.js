...
async function checkDeviceStatus() {
  try {
    ...
    if (!response.ok) {
      console.error("디바이스 상태 확인 실패:", response.status);
      return false;
    }

    const data = await response.json();
    console.log("모든 디바이스:", data.devices); // 디버깅용

    const device = data.devices.find((d) => d.iden === DEVICE_IDEN);

    if (!device) {
      console.error(
        "설정된 디바이스를 찾을 수 없습니다. Device ID:",
        DEVICE_IDEN
      );
      return false;
    }

    console.log("선택된 디바이스 정보:", device); // 디버깅용
    ...
    console.error("디바이스 상태 확인 중 상세 오류:", error);
    return false;
  }
}
