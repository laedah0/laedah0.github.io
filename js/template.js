		...
		$.each(_data, function(entryIndex, entry) {
			...
			*/
			...
			switch(entry.tag)
			{
				...
				case '주문' :
					//htmlBtnGroup5
					htmlBtnGroup5 += '<button type="button" class="button button--moema button--inverted button--text-thick button--size-s" id="favoritBtn'+entryIndex+'" onClick="clickFavoritTempBtn('+entryIndex+')">'+entry.key+'</button> ';
					break;
				case '배송' :
					//htmlBtnGroup6
					htmlBtnGroup6 += '<button type="button" class="button button--moema button--inverted button--text-thick button--size-s" id="favoritBtn'+entryIndex+'" onClick="clickFavoritTempBtn('+entryIndex+')">'+entry.key+'</button> ';
					break;
				default :
					break;
			}
