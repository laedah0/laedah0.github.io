    ...
    let inputEvent = function(e) {
        ...
        for (i = 0; i < _arr.length; i++) {
            // 배열의 요소를 현재 input의 value의 값만큼 자른 후, 같으면 추가한다. 
           	//console.log(_arr[i],val);
            if (_arr[i][0].substr(0, val.length).toUpperCase() == val.toUpperCase()) {
                b = document.createElement("DIV");
                // value의 값 만큼 굵게 표시
                b.innerHTML = "<strong>" + _arr[i][0].substr(0, val.length) + "</strong>";
                b.innerHTML += _arr[i][0].substr(val.length);

                //가격 더해서 표기
                b.innerHTML += "mm(단가&nbsp;" + _arr[i][1] + "원)";

                b.innerHTML += "<input type='hidden' value='" + _arr[i][0] + "'>";
                //                        console.log(b);
